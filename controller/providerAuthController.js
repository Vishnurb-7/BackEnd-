...
const logout = async (req, res) => {
  ...
  const refreshTokens2 = refreshTokens.filter(
    ...
    if (provider) {
      ...
      if (validPassword) {
        ...
        const refreshToken = await jwt.sign(
          data,
          process.env.PROVIDER_REFRESH_SECRET,
          { expiresIn: "15d" }
        );
...
const generateUserAccessToken = (user) => {
  return jwt.sign(user, process.env.PROVIDER_ACCESS_SECRET, {
    expiresIn: "5m",
  });
};
